...
#Ajout Alex
@register.filter
def accept_request_url(user, status):
    if isinstance(status, RelationshipStatus):
        status = status.from_slug
    return reverse('relationship_accept', args=[user.username, status])
    
@register.filter
def decline_request_url(user, status):
    if isinstance(status, RelationshipStatus):
        status = status.from_slug
    return reverse('relationship_decline', args=[user.username, status])
