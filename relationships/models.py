...
#    def _get_from_query(self, status):
    def _get_from_query(self, status, accepted):
...
#    def _get_to_query(self, status):
    def _get_to_query(self, status, accepted):
#Ajouter la clause to_users__accepted=True pour ne retourner que les relationships validees
        return dict(
...
#    def get_relationships(self, status, symmetrical=False):
    def get_relationships(self, status, accepted=True, symmetrical=False):
        """
...
#        query = self._get_from_query(status)
    ...
    def get_related_to(self, status, accepted=True):
        """
        Returns a QuerySet of user objects which have created a relationship to
        the given user.
        """
	print self._get_to_query(status, accepted)
        return User.objects.filter(**self._get_to_query(status, accepted))
